 ...
 * @version 1.0
 */
public class AlterationIDDlg extends JDialog implements ScrollProcessor {
   ...
   * @param    String[] grades
   */
  public AlterationIDDlg(Frame frame, IApplicationManager theAppMgr, Hashtable htAlterationIDs
      , String sHeader) {
    super(frame, "", true);
    try {
      model = new AlterationIDModel(sHeader);
      ...
      loadAlterationIDs(htAlterationIDs);
      this.setModal(true);
  ...
  void jbInit()
    ...
    tblAlterationIDList.addMouseListener(new java.awt.event.MouseAdapter() {
    ...
    tblAlterationIDList.addComponentListener(new java.awt.event.ComponentAdapter() {

      /**
       * @param e
       */
      public void componentResized(ComponentEvent e) {
        tblAlterationIDList_componentResized(e);
      }
    });
  ...
  void btnScrollUp_actionPerformed(ActionEvent e) {
    model.prevPage();
    tblAlterationIDList.setRowSelectionInterval(0, 0);
    fireButtonClick(e);
  }
   ...
   * @return
   */
  public String getSelectedAlterationID() {
    return sSelectedAlterationID;
  }
  ...
  void btnOK_actionPerformed(ActionEvent e) {
    int row = tblAlterationIDList.getSelectedRow();
    if (row < 0) {
      theAppMgr.showErrorDlg(com.chelseasystems.cr.util.ResourceManager.getResourceBundle().
          getString("You must first select a Alteration type."));
      return;
    }
    sSelectedAlterationID = model.getSelectedAlterationID(row);
    ok = true;
    dispose();
  }
   ...
   * @param e
   */
  void tblAlterationIDList_componentResized(ComponentEvent e) {
    model.setColumnWidth(tblAlterationIDList);
    model.setRowsShown(tblAlterationIDList.getHeight() / tblAlterationIDList.getRowHeight());
  }
}
