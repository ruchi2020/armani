...
import com.chelseasystems.cs.swing.model.CouponModel;
import java.util.*;


/**
 */
public class CouponDlg extends JDialog implements ScrollProcessor {
  ...
  protected CouponModel model = new CouponModel();
  JCMSTable tblCoupon = new JCMSTable(model, JCMSTable.SELECT_ROW);
   ...
   * @param    String[] grades
   */
  public CouponDlg(Frame frame, IApplicationManager theAppMgr, Hashtable couponArray) {
    super(frame, "Coupon", true);
    try {
      this.theAppMgr = theAppMgr;
      ttip = new JCMSToolTip(this, theAppMgr.getTheme(), "");
      ttip.setType(JCMSToolTip.FOLLOWING);
      jbInit();
      tblCoupon.setAppMgr(theAppMgr);
      //System.out.println("the value of r: " + r);
      pack();
      setSize((int)(r * 800), (int)(r * 400));
      loadCoupons(couponArray);
      this.setModal(true);
   ...
   * @exception Exception
   */
  public void loadCoupons(Hashtable htLabels)
      throws Exception {
    if (htLabels.isEmpty())
      return;
    model.addCoupon(htLabels);
  }
  ...
  void jbInit()
    ...
    tblCoupon.addMouseListener(new java.awt.event.MouseAdapter() {
      ...
      public void mouseClicked(MouseEvent e) {
        tblCoupon_mouseClicked(e);
      }
    });
    tblCoupon.addComponentListener(new java.awt.event.ComponentAdapter() {

      /**
       * @param e
       */
      public void componentResized(ComponentEvent e) {
        tblCoupon_componentResized(e);
      }
    });
    ...
    jPanel6.add(tblCoupon.getTableHeader(), BorderLayout.NORTH);
    jPanel6.add(tblCoupon, BorderLayout.CENTER);
  ...
  void btnScrollUp_actionPerformed(ActionEvent e) {
    model.prevPage();
    tblCoupon.setRowSelectionInterval(0, 0);
    fireButtonClick(e);
  }
   ...
   * @param e
   */
  void tblCoupon_mouseClicked(MouseEvent e) {
  ...
  void btnOK_actionPerformed(ActionEvent e) {
    ...
    if (row < 0) {
      theAppMgr.showErrorDlg(com.chelseasystems.cr.util.ResourceManager.getResourceBundle().
          getString("You must first select a coupon type."));
      return;
    }
    selectedCoupon = model.getSelectedCoupon(row);
    ok = true;
    dispose();
  }
   ...
   * @param e
   */
  void tblCoupon_componentResized(ComponentEvent e) {
    model.setColumnWidth(tblCoupon);
    model.setRowsShown(tblCoupon.getHeight() / tblCoupon.getRowHeight());
  }
}
