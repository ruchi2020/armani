 ...
 * @version 1.0
 */
public class CMSNoReturnLineItemDetailPriceEngine extends LineItemDetailPriceEngine {
	 ...
	 */
	public CMSNoReturnLineItemDetailPriceEngine() {
	}
	 ...
	 * @return Currency
	 */
	public ArmCurrency calcReductionAmountForCompositeTransactionDiscount(Vector appliedReductions, ArmCurrency baseMarkdown, Vector baseMarkdownReductions) {
		...
		try {
			...
			ArmCurrency totalAmountOff = new ArmCurrency(0.0d);
				...
				if (!discount.isDiscountPercent() || discount instanceof RewardDiscount) {
					CMSCompositePOSTransaction trans = (CMSCompositePOSTransaction) this.getCompositeTransaction();
					double compositeTotalThresholdAmount = trans.getReturnRetailAmount().subtract(trans.getCompositeTotalDealMkdnAmount()).doubleValue();
					totalAmountOff = new ArmCurrency(Math.min(discount.getAmount().getDoubleValue(), compositeTotalThresholdAmount));
			...
			ArmCurrency discountAmount = (this.createDiscountPriceEngine(discount)).calculateDiscountAmount(this.getLineItem(), aDiscountAmount);
	 ...
	 * @return Currency
	 */
	public ArmCurrency calcReductionAmountForLineItemDiscount(Vector appliedReductions, ArmCurrency baseMarkdown, Vector baseMarkdownReductions) {
	ArmCurrency aDiscountAmount = new ArmCurrency(0.0);
		...
		try {
			...
			for (int i = 0; i < aDiscounts.length; i++) {
				if (aDiscounts[i].getType().equals("BY_PRICE_DISCOUNT")) {
					try {
						CMSNoReturnLineItem returnItem = (CMSNoReturnLineItem) this.getLineItem();
						if (returnItem.getPriceDiscount() != null) {
							priceDiscount = (Discount) returnItem.getPriceDiscount().clone();
							priceDiscount.setAmount(returnItem.getItemSellingPrice().subtract(this.getLineItemDetail().getDealMarkdownAmount()).subtract(priceDiscount.getAmount()));
						ArmCurrency discountAmount = (this.createDiscountPriceEngine(priceDiscount)).calculateDiscountAmount(this.getLineItem(), aDiscountAmount);
							aDiscountAmount = aDiscountAmount.add(discountAmount);
							if (discountAmount.equals(new ArmCurrency(0.0)))
								continue;
							CMSReduction reduction = new CMSReduction(discountAmount, priceDiscount.getType() + " Discount");
			...
			for (int index = 0; index < aDiscounts.length; index++) {
				...
				if (returnItem.getPriceDiscount() == discount)
					continue;
				// added by nupura
				if (discount instanceof CMSEmployeeDiscount && !((CMSEmployeeDiscount) discount).getIsOverridden()) {
				ArmCurrency retailPrice = returnItem.getExtendedRetailAmount();
				ArmCurrency sellingPrice = returnItem.getItem().getSellingPrice().multiply(returnItem.getQuantity().intValue());
					sellingPrice = sellingPrice.subtract(((CMSNoReturnLineItem) returnItem).getExtendedDealMarkdownAmount());
				ArmCurrency markdown = retailPrice.subtract(sellingPrice);
					...
					if (markdown.greaterThan(new ArmCurrency(0.0d))) {
						...
						((CMSEmployeeDiscount) discount).doSetPercent(((CMSEmployeeDiscount) discount).getNormalDiscountPrecent());
					}
				}
				//ends here
			ArmCurrency discountAmount = (this.createDiscountPriceEngine(discount)).calculateDiscountAmount(this.getLineItem(), aDiscountAmount);
	...
	ArmCurrency totalReduction = new ArmCurrency(0.0d);
		...
		if ((item.isRedeemable()) || ((MiscItemManager.getInstance().isMiscItem(item.getId())) && !LineItemPOSUtil.isNotOnFileItem(item.getId()))) {
			totalReduction = new ArmCurrency(0.0);
			return totalReduction;
		}
		try {
		ArmCurrency reduction1 = this.getBaseMarkdownAmount(appliedReductions1);
		ArmCurrency reduction2 = this.calcReductionAmountForCompositeTransactionDiscount(appliedReductions2, reduction1, appliedReductions1);
			reduction1 = reduction1.add(reduction2);
		ArmCurrency reduction3 = this.calcReductionAmountForLineItemDiscount(appliedReductions3, reduction1, appliedReductions1);
	...
	public POSLineItemDetail[] getSortedLineItemDetailArray(ArmLineDiscount lineDiscount) {
		...
		for (Iterator aLineItemGroupingList = this.getLineItem().getTransaction().getLineItemGroupings(); aLineItemGroupingList.hasNext();) {
			...
			for (Iterator iterator = lineItemDetails.iterator(); iterator.hasNext();) {
				POSLineItemDetail returnLineItemDetail = (POSLineItemDetail) iterator.next();
				...
				for (int index = 0; index < discLineItemDetails.length; index++) {
					if (returnLineItemDetail == discLineItemDetails[index]) {
