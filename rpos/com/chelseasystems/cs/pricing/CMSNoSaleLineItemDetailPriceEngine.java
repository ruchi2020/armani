 ...
 * @version 1.0
 */
public class CMSNoSaleLineItemDetailPriceEngine extends LineItemDetailPriceEngine {

    private static ConfigMgr itemCfg = new ConfigMgr("item.cfg");
  /**
   * put your documentation comment here
   */
  public CMSNoSaleLineItemDetailPriceEngine() {
  }
    ...
    try {
      ...
      for (int i = 0; i < aDiscounts.length; i++) {
        if (aDiscounts[i].getType().equals("BY_PRICE_DISCOUNT")) {
          try {
            CMSNoSaleLineItem saleItem = (CMSNoSaleLineItem)this.getLineItem();
      ...
      for (int index = 0; index < aDiscounts.length; index++) {
        ...
        if (discount instanceof CMSEmployeeDiscount
          ...
          sellingPrice = sellingPrice.subtract(((CMSNoSaleLineItem)saleItem).
