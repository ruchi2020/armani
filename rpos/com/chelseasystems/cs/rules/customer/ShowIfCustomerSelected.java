 ...
 *
 * <p>Description: This business rule check whether customer is selected or not</p>
 *
...
/* History:-
 ...
 +--------+------------+-----------+------------------+------------------------------+
 */
public class ShowIfCustomerSelected extends Rule {

  /**
   * Default Constructor
   */
  public ShowIfCustomerSelected() {
  }
  ...
  private RulesInfo execute(CMSMenuOption cmsmenuoption, Employee employee, Store store) {
    try {
      if (!((String)CMSApplet.theAppMgr.getStateObject("CUST_MGMT_MODE")).equalsIgnoreCase("MODIFY"))
        return new RulesInfo("Cant modify customer that doesnt exist");
    } catch (Exception ex) {
