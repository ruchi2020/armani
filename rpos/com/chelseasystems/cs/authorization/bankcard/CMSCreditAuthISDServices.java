...
import com.chelseasystems.cs.authorization.*;
...
/**
 *
 * <p>Title: CMSCreditAuthISDServices</p>
 *
 * <p>Description: This class specifies the ISD Service implementation
 * of CreditAuthServices.</p>
 *
 ...
 * @version 1.0
 */
public class CMSCreditAuthISDServices extends CMSCreditAuthServices implements IConfig {
  ...
  /**
   * Constuctor for CreditAuthISDServices
   **/
  public CMSCreditAuthISDServices() {
  ...
  /*
   * Returns reply for each credit authorization request
   * @param   request  A string array containing credit requests
   * @return           A string array containing credit replies
   */
  public Object[] getAuth(Object[] request) {
    ...
    double testInt = 0.0;
    if (verboseMode) {
      System.out.println("CreditAuthISDServices.getAuth(): No requests = " + request_length);
    }
    for (int i = 0; i < request_length; i++) {
      try {
        if (!localTestMode) {
          // Initialize reply in case exception is thrown in getCreditAuth() method.
          reply[i] = "";
          tempReply[i] = ISDServiceManager.getCurrent().getCreditAuth((String)request[i]);
          if (tempReply[i] != null) {
            reply[i] = tempReply[i];
            // Print out debug information if verbose mode
            if (verboseMode) {
              System.out.println("request #" + i + " to ISDService >" + request[i] + "<");
              System.out.println("reply #" + i + " from ISDService >" + reply[i] + "<");
              verifyRequestCheck((String)request[i]);
              //try {
              //   String fileName = AppManager.getLocalFile("credit.out");
              //   RandomAccessFile outputFile = new RandomAccessFile(fileName, "rw");
              //   outputFile.seek(outputFile.length());
              //   outputFile.writeBytes("Server string>" + request[i] + "<");
              //} catch (Exception e) {
        ...
        // Local test mode will return approval codes according to the dollar
        // amount for the credit card payment.
        // Anything less than or equal to $101.00 or greater than $105.00 will return
        // an 01=approval.  An amount of $102.00 will return 02=decline, $103.00
        // will return 03=call center, $104.00 will return 04=timeout and $105.00
        // will return 05=system error.
        if (localTestMode) {
          ...
          if (verboseMode) {
            ...
            System.out.println("request #" + i + " to ISDService >" + request[i] + "<");
            verifyRequestCheck((String)request[i]);
            System.out.println("StringDollarAmount for TESTMODE= " + strDollarAmount
                + "DollarAmount=" + dollarAmount.toString() + "TESTINT=" + testInt);
          }
          if (testType.equals("CK")) {
            // GET TEST CHECK APPROVAL payment1=approved, 2=declined, 3=call, 4=timeout, 5=system failure
            reply[i] = createCKTestApproval(testInt);
            if (verboseMode) {
              System.out.println("TESTING FOR TYPE=" + testType);
              System.out.println("CreditAuthISDServices.getAuth():  " + i + "reply length = "
                  + reply[i].length() + "\n\t reply = " + (reply == null ? "NULL" : reply[i]));
              System.out.println("\nVERIFYING CHECK REQUEST REPLY");
              verifyResponseCheck(reply[i]);
              LoggingServices.getCurrent().logMsg("CreditAuthISDServices", "getAuth()"
                  , "Setting the off-line CLIENT_IMPL parameter from the config file for"
                  + " the implementation of CreditAuthServices in credit_auth.cfg.",
                  " ", LoggingServices.CRITICAL);
            }
          } else {
            // GET TEST CREDIT CARD APPROVAL payment1=approved, 2=declined, 3=call, 4=timeout, 5=system failure
            reply[i] = createCCTestApproval(testInt);
            if (verboseMode) {
              System.out.println("TESTING FOR TYPE=" + testType);
              System.out.println("CreditAuthISDServices.getAuth():  " + i + "reply length = "
                  + reply[i].length() + "\n\t reply = " + (reply == null ? "NULL" : reply[i]));
              verifyResponseCreditCard(reply[i]);
            }
          }
        } // End of if ( localTestMode )
      } catch (Exception e) { // End of Try
        LoggingServices.getCurrent().logMsg(getClass().getName(), "getAuth()"
            , "Exception occured: " + e, "Verify ISDService running", LoggingServices.MAJOR);
        System.err.println("CreditAuthISDServices.getAuth(): exception = " + e);
        return reply;
      }
  ...
  /**
   * This method call ISDServiceManager for cleanup if the
   * CreditAuthServer dies
   */
  protected void finalize()
      throws Throwable {
    ISDServiceManager.getCurrent().finalize();
  }

  /**
   * This is helper method for determining if the system is available
   * to return to up-time mode.
   **/
  public boolean ping()
  ...
  /**
   * Method which checks the fields within the credit card reply
   * to determine if they are correct.  Used for display output to GUI.  To
   * display output to log file, use verifyResponseCreditCardLog().
   * @param result    String containing credit reply
   */
  private void verifyResponseCreditCard(String result) {
    ...
    fieldOK(1, 210, 211, CHARACTERS, result, "Card Type B-Bus, R-Corp, S-Purchasing");
    fieldOK(3, 211, 214, DIGITS, result, "Currency Code");
    fieldOK(1, 214, 215, CHARACTERS, result, "Payment Service Indicator");
    fieldOK(15, 215, 230, CHARACTERS, result, "Transaction Identifier/Bank Net Info");
    ...
    fieldOK(6, 242, 248, CHARACTERS, result, "Local Transaction Time 6 CHAR hhmmss");
    fieldOK(6, 248, 254, DIGITS, result, "System Trace Audit Number 6 DIGITS");
  ...
  /**
   * Method which checks the fields within the check reply
  ...
  private void verifyResponseCheck(String result) {
    ...
    fieldOK(1, 190, 191, CHARACTERS, result, "Check Truncation Indicator 1 CHAR");
    fieldOK(22, 191, 213, CHARACTERS, result, "Transaction ID 22 CHARS");
    fieldOK(5, 213, 218, CHARACTERS, result, "Service Charge 5 CHARS");
  }

  /**
   * Method which checks the fields within the check request
   * to determine if they are correct.
   * @param request    String containing credit request
   */
  ...
  private boolean fieldOK(int length, int start, int end, int data_type, String str, String name) {
    if ((str.length() < end) || ((end - start) != length)) {
      System.out.println("Field: " + name + " length too short, length = " + str.length()
          + " required length = " + length + " start = " + start + " end = " + end);
      return false;
    }
  ...
  /**
   * Method to create a test string used in testing only.  Not needed in production.
  ...
  private String createCCTestApproval(double i) {
    ...
    app.append("CC"); // Message Identifier CC=credit card, CA=credit app, CK=check
    ...
    if (i <= 101.00 || i >= 106.00) {
      ...
      app.append("01"); // 01=Approved, 02=Declined, 03=Refer, 04=Timeout, 05=System Failure
    }
    if (i == 102.00) {
      app.append("D"); // A=approved, D=declined, O=other, N=other
      app.append("02"); // 01=Approved, 02=Declined, 03=Refer, 04=Timeout, 05=System Failure
      System.out.println("In createTestApproval" + app);
    }
    if (i == 103.00) {
      app.append("O"); // A=approved, D=declined, O=other, N=other
      app.append("03"); // 01=Approved, 02=Declined, 03=Refer, 04=Timeout, 05=System Failure
    }
    if (i == 104.00) {
      app.append("O"); // A=approved, D=declined, O=other, N=other
      app.append("04"); // 01=Approved, 02=Declined, 03=Refer, 04=Timeout, 05=System Failure
    }
    if (i == 105.00) {
      app.append("O"); // A=approved, D=declined, O=other, N=other
      app.append("05"); // 01=Approved, 02=Declined, 03=Refer, 04=Timeout, 05=System Failure
    }
    // app.append("0");
    //app.append("04");
    app.append("12345678901234567890123456789012345678"); // Response Message
    ...
    app.append("1234567890123456789012345678901234567890123456789012345678790"); //60         // EBT Response Test 141 Char
    ...
    //String str = new String("9999CC12345678901234560031234567890123456789123456789011099003300001234567890123456789019991225123456A0112345678901234567890123456789012345678003800M6666669999123412345678901234567890123456712340012345678901200S8400123456789012345123499122501");
    return app.toString();
  }
  ...
  private String createCKTestApproval(double i) {
    ...
    //app.append("12340012345678901200S8400123456789012345123499122501");   // rest
    //String str = new String("9999CC12345678901234560031234567890123456789123456789011099003300001234567890123456789019991225123456A0112345678901234567890123456789012345678003800M6666669999123412345678901234567890123456712340012345678901200S8400123456789012345123499122501");
    return app.toString();
  }
