 ...
 */
public class PresaleDetailsReport implements java.io.Serializable {
	static final long serialVersionUID = -6572138789941892281L;
	public PresaleDetailsReportBean bean;
	...
	public String[] toStringArray() {
		...
		array[6] = (bean.dateSoldReturn == null) ? "" : new SimpleDateFormat(ReportConstants.DATE_FORMAT_SLASH_MDY).format(bean.dateSoldReturn);
	 ...
	 * @return
	 */
	public static String[] getTotalArray(String colZero, int totalOrigQty, ArmCurrency totalOrigAmt, int totalRemainQty, ArmCurrency totalRemainAmt) {
		String[] array = new String[10];
		array[0] = colZero;
		array[1] = "";
		array[2] = "";
	 ...
	 * @return
	 */
	public static PresaleDetailsReport[] filterByAssociate(PresaleDetailsReport[] all, String associateId) {
		...
		return (PresaleDetailsReport[]) list.toArray(new PresaleDetailsReport[0]);
	}
	 ...
	 * @return
	 */
	public static PresaleDetailsReport[] filterOpenByAssociate(PresaleDetailsReport[] all, String associateId) {
		if (all == null || all.length == 0) {
			return new PresaleDetailsReport[0];
		}
		List list = new ArrayList();
		for (int i = 0; i < all.length; i++) {
			if (all[i].getItemStatus().equals("OPEN")) {
				if (associateId == null || associateId.length() == 0 || all[i].getAssociateId().equals(associateId.trim())) {
					list.add(all[i]);
				}
			}
		}
	 ...
	 * @return
	 */
	public static PresaleDetailsReport[] filterCloseByAssociate(PresaleDetailsReport[] all, String associateId) {
		if (all == null || all.length == 0) {
			return new PresaleDetailsReport[0];
		}
		List list = new ArrayList();
