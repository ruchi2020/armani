...
package com.ga.cs.swing.report.presaledetails;
...
import com.ga.cs.swing.model.PresaleDetailsReportModel;
 ...
 */
public class PresaleDetailsReportApplet extends BaseReportApplet implements Reportable {

	static final long serialVersionUID = 0;

	public static final String MAIN_TITLE = "Presale Details Report";
	public static final boolean DEBUG = true;
	private static int FILTER_BY_ALL = 0;
	private static int FILTER_BY_OPEN = 1;
	private static int FILTER_BY_CLOSE = 2;
	private PresaleDetailsReport[] data = null;
	private List rows;
	 ...
	 * put your documentation comment here
	 */
	public PresaleDetailsReportApplet() {
		super();
		setTitle(MAIN_TITLE);
		this.model = new PresaleDetailsReportModel();
	...
	public void populateScreen(Date startDate, Date endDate) {
		try {
			ReportUtils.logInfoTsMsg("PRESALE DETAILS REPORT POPULATE SCREEN");
			this.model.clear();
			this.model.setFullTitle(this.getPrintTitle());
			this.lblTitle.setText(this.getScreenTitle());
			theAppMgr.showMenu(MenuConst.PRESALE_DETAIL_REPORT, theOpr, IReceiptAppManager.PREV_CANCEL_BUTTON);
			this.prepareRows();
			...
			ReportUtils.logInfoTsMsg("PRESALE DETAILS REPORT: Exception in method-> POPULATESCREEN " + ex + " msg=" + ex.getMessage());
			ex.printStackTrace();
		}
	}
	...
	public String getScreenName() {
		return (res.getString("Presale Detail Rpt"));
	}
	...
	private void fetchData(String storeId, Date begin, Date end) {
		try {
			this.data = GAReportHelper.getPresaleDetails(theAppMgr, storeId, begin, end);
			if (DEBUG) {
				System.out.println("\n\ndata: " + data + " of size " + data.length);
				for (int i = 0; i < data.length; i++) {
					PresaleDetailsReport row = data[i];
					System.out.println("  row " + i + ": " + row);
				}
			}
		} catch (Exception ex) {
			ReportUtils.logInfoTsMsg("PRESALE DETAILS REPORT: Exception in method-> FETCHDATA " + ex + " msg=" + ex.getMessage());
			ex.printStackTrace();
		}
	}
	 ...
	 * @return
	 */
	private PresaleDetailsReport[] applyCurrentReportFilter() {
		if (currentReportFilter == FILTER_BY_OPEN) {
			return PresaleDetailsReport.filterOpenByAssociate(this.data, currentAssociateId);
		} else if (currentReportFilter == FILTER_BY_CLOSE) {
			return PresaleDetailsReport.filterCloseByAssociate(this.data, currentAssociateId);
		} else {
			return PresaleDetailsReport.filterByAssociate(this.data, currentAssociateId);
		}
	}
	...
	private void prepareRows() {
		PresaleDetailsReport[] currentData = this.applyCurrentReportFilter();
		Map reportRows = new TreeMap();
		for (int i = 0; i < currentData.length; i++) {
			PresaleDetailsReport report = currentData[i];
		...
		// Prepare row data (header row, detail rows and total row for each presale)
		...
		for (Iterator iter = reportRows.keySet().iterator(); iter.hasNext();) {
			...
			PresaleTotals totals = getTotals(reports);
			for (Iterator iterator = reports.iterator(); iterator.hasNext();) {
				PresaleDetailsReport report = (PresaleDetailsReport) iterator.next();
				if (!headerAdded) {
			...
			String[] totalRow = PresaleDetailsReport.getTotalArray(res.getString("Total Qty/Amt:"), totals.totalOrigQty, totals.totalOrigAmt, totals.totalRemainQty, totals.totalRemainAmt);
			rows.add(totalRow);
			rows.add(new String[] { "", "", "", "", "", "", "", "", "", "" });
			totalSet.add(totals);
		}
	...
	ArmCurrency grandTotalRemainAmt = null;
		try {
			for (Iterator iter = totalSet.iterator(); iter.hasNext();) {
				PresaleTotals totals = (PresaleTotals) iter.next();
		...
		String[] grandTotalRow = (grandTotalOrigQty == 0) ? new String[] { res.getString("Grand Total:"), "", "", "0", "0", "", "", "0", "0", "" } : PresaleDetailsReport.getTotalArray(
	...
	public void appButtonEvent(CMSActionEvent anEvent) {
		...
		if (sAction.equals("PRINT")) {
			...
			reportPrinter.print();
		} else if (sAction.equals("ALL_PRESALE_DETAIL")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_ALL;
			currentAssociateId = null;
			populateScreen(startDate, endDate);
		} else if (sAction.equals("CLOSED_PRESALE_DETAIL_ONLY")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_CLOSE;
			currentAssociateId = null;
			populateScreen(startDate, endDate);
		} else if (sAction.equals("OPEN_PRESALE_DETAIL_ONLY")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_OPEN;
			currentAssociateId = null;
			populateScreen(startDate, endDate);
		} else if (sAction.equals("ALL_PRESALE_DETAIL_BY_ASSOCIATE")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_ALL;
			this.enterAssocaiteId();
		} else if (sAction.equals("CLOSED_PRESALE_DETAIL_BY_ASSOCIATE")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_CLOSE;
			this.enterAssocaiteId();
		} else if (sAction.equals("OPEN_PRESALE_DETAIL_BY_ASSOCIATE")) {
			currentReportFilter = PresaleDetailsReportApplet.FILTER_BY_OPEN;
			this.enterAssocaiteId();
		}
	}
	...
	// additional header row and total row for each presale (ie. transaction).
	 ...
	 * @return
	 */
	private PresaleTotals getTotals(List reports) {
		try {
			PresaleTotals totals = new PresaleTotals();
			for (Iterator iter = reports.iterator(); iter.hasNext();) {
				PresaleDetailsReport report = (PresaleDetailsReport) iter.next();
	...
	private String getScreenTitle() {
		...
		if (this.currentReportFilter == PresaleDetailsReportApplet.FILTER_BY_OPEN) {
			sb.append(res.getString("OPEN"));
			sb.append(" ");
		} else if (this.currentReportFilter == PresaleDetailsReportApplet.FILTER_BY_CLOSE) {
			sb.append(res.getString("CLOSED"));
			sb.append(" ");
		}
		sb.append(res.getString("Presale Details"));
		sb.append(" ");
